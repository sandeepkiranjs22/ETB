...
class yaml_pipeline:
    ...
    def restriction_check_int(self,rest,att,data):
            ...
            if pd.isnull(data[colname][m])==True:
                ...
                # if the recommended product has restrictions
                    ...
                    for i in range(len_cat):
                        ...
                        #get the type of restriction whether list or int
                        data_type=(yaml_file[cat][i]['type'])

                        if data_type=="int":
                            # sending in entire list,column name
                            val=self.restriction_check_int(yaml_file[cat][i],yaml_file[cat][i]['attribute'],data.iloc[m])
                            #print("int:"+str(travel[cat][i]['attribute'])+" "+str(val))
                            if val==1:
                                c=c+1
                                continue
                            else:
